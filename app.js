(() => {
  ...
  function displayName(p){ return app.state?.customNames?.[p.id] || p.name; }
  ...
  function renderView(){
    ...
    hydrateCustomArt(root);
    bindViewHandlers();
  }

  function artHTML(p, cls=''){
    if(p.image){
      return `<img class="${cls}" data-art-id="${p.id}" src="${esc(p.image)}" alt="${esc(displayName(p))} packaging">`;
    }
  ...
  function openProduct(id){
    ...
    openModal(`<div class="modal-handle"></div><div class="modal-head"><div><div class="eyebrow">${esc(franchiseById(p.franchise).name)}</div><h2>${esc(displayName(p))}</h2><p>${esc(manufacturerById(p.manufacturer).name)} • ${esc(p.releaseStructure||p.category)}</p></div><button class="icon-btn" data-modal-action="close">×</button></div>
      ...
      <div class="modal-actions">
        ${inv.stockroom>0?'<button class="btn btn--secondary" data-modal-action="restock-product" data-id="'+id+'">RESTOCK THIS PRODUCT</button>':''}
        <button class="btn btn--secondary" data-modal-action="rename-product" data-id="${id}">EDIT DISPLAY NAME</button>
        <label class="btn btn--secondary" style="text-align:center;display:block">REPLACE LOCAL ARTWORK<input type="file" accept="image/*" data-art-upload="${id}" hidden></label>
        <button class="btn btn--ghost" data-modal-action="reset-custom" data-id="${id}">RESET NAME + ARTWORK</button>
      </div>`);
    hydrateCustomArt($('#modalLayer'));
  ...
  function bindModalProductControls(){
    ...
    $$('[data-art-upload]', $('#modalLayer')).forEach(input=>input.addEventListener('change', async e=>{
      const file=e.target.files?.[0]; if(!file) return;
      if(file.size>6*1024*1024){toast('Please use an image under 6 MB.','bad');return;}
      const dataUrl=await fileToDataURL(file);
      await artDB.set(input.dataset.artUpload,dataUrl);
      toast('Local artwork replaced for this save.','good');
      openProduct(input.dataset.artUpload);
    }));
  }
  ...
  function renameProduct(id){
    const p=productById(id); if(!p)return;
    const current=displayName(p);
    openModal(`<div class="modal-handle"></div><div class="modal-head"><div><div class="eyebrow">LOCAL CUSTOMISATION</div><h2>Edit display name</h2><p>This changes only your save. Internal product IDs and simulation data stay unchanged.</p></div></div><label class="field-label" for="renameInput">DISPLAY NAME</label><input class="text-input" id="renameInput" maxlength="60" value="${esc(current)}"><div class="modal-actions"><button class="btn btn--hero" data-modal-action="save-rename" data-id="${id}">SAVE NAME</button><button class="btn btn--secondary" data-modal-action="close">CANCEL</button></div>`);
    setTimeout(()=>$('#renameInput')?.focus(),40);
  }
  ...
  function bindModalActions(){
    $$('[data-modal-action]', $('#modalLayer')).forEach(el=>el.addEventListener('click',()=>{
      ...
      if(a==='rename-product') renameProduct(id);
      if(a==='reset-custom') resetCustom(id);
      if(a==='save-rename') saveRename(id);
  ...
  async function resetCustom(id){
    delete app.state.customNames[id]; save(); await artDB.remove(id); toast('Default name and artwork restored.','good'); openProduct(id);
  }
  function saveRename(id){
    const v=$('#renameInput')?.value.trim(); if(!v) return;
    app.state.customNames[id]=v; save(); toast('Display name updated for this save.','good'); openProduct(id);
  }
  ...
  const artDB = {
    ...
    async open(){
      ...
      return new Promise(resolve=>{
        const req=indexedDB.open('ToyStoreTycoonAssets',1);
        req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains('art'))db.createObjectStore('art');};
        req.onsuccess=()=>{this.db=req.result;resolve(this.db);}; req.onerror=()=>resolve(null);
      });
    },
    async get(id){const db=await this.open();if(!db)return null;return new Promise(resolve=>{const tx=db.transaction('art','readonly');const r=tx.objectStore('art').get(id);r.onsuccess=()=>resolve(r.result||null);r.onerror=()=>resolve(null);});},
    async set(id,val){const db=await this.open();if(!db)return;return new Promise(resolve=>{const tx=db.transaction('art','readwrite');tx.objectStore('art').put(val,id);tx.oncomplete=()=>resolve();tx.onerror=()=>resolve();});},
    async remove(id){const db=await this.open();if(!db)return;return new Promise(resolve=>{const tx=db.transaction('art','readwrite');tx.objectStore('art').delete(id);tx.oncomplete=()=>resolve();tx.onerror=()=>resolve();});}
  };

  async function hydrateCustomArt(root){
    if(!root) return;
    const nodes=$$('[data-art-id]',root);
    for(const node of nodes){
      const id=node.dataset.artId, custom=await artDB.get(id); if(!custom) continue;
      if(node.tagName==='IMG'){node.src=custom;}
      else {const img=document.createElement('img');img.dataset.artId=id;img.src=custom;img.alt=`${displayName(productById(id))} custom artwork`;img.style.cssText='width:100%;height:100%;object-fit:contain;border-radius:inherit;background:#f4f5f8';node.replaceWith(img);}
    }
  }
