(() => {
  ...
  function renderStore(){
    ...
    return `
      ...
      <section class="section">
        <div class="section-title-row"><div class="section-title">Store departments</div><div class="section-sub">9 DISPLAY BAYS</div></div>
  ...
  function renderMiniProduct(id){
    ...
    return `<button class="product-mini" data-product="${id}" style="text-align:left;color:inherit;border-color:rgba(255,255,255,.08)">
      <div class="product-mini-art">${artHTML(p)}</div>
      <div class="mini-status-row"><span class="pill ${st.cls} mini-badge">${st.label}</span></div>
    ...
    return `<article class="card catalog-card">
      ...
      <div class="catalog-body">
        ...
        <div class="catalog-pricing">
          <div class="catalog-price-stat"><small>WHOLESALE</small><strong>${money2(p.wholesale)}</strong></div>
          <div class="catalog-price-stat catalog-price-stat--rrp"><small>RRP</small><strong>${money2(p.rrp)}</strong></div>
        </div>
        <div class="catalog-status"><span class="pill ${trend>1.2?'pill--hot':'pill--info'}">${availability}</span></div>
  ...
  function renderMarketFeedItem(f){
    const p = f.productId ? productById(f.productId) : null;
    return `<div class="card feed-card ${p?'feed-card--product':''}">
      ${p ? `<div class="feed-product-art-wrap">${artHTML(p,'feed-product-art')}</div>` : `<div class="feed-icon">${f.icon}</div>`}
      <div class="feed-copy"><h3>${esc(f.title)}</h3><p>${esc(f.text)}</p><div class="feed-time">${esc(f.time)}</div></div>
    </div>`;
  }
  ...
  function renderMarket(){
    ...
    return `<div class="page-head"><div><div class="eyebrow">LOCAL MARKET</div><h2>What people are saying</h2><p>Customer chatter, product momentum and rival behaviour are real clues to demand.</p></div></div>
      <section class="section" style="margin-top:0"><div class="section-title-row"><div class="section-title">Trending now</div><div class="section-sub">LOCAL DEMAND</div></div>
        <div class="grid-2">${trenders.map(({p,t})=>`<button class="card market-product-card" data-product="${p.id}" style="text-align:left;color:inherit;border-color:var(--line)"><div class="market-product-thumb">${artHTML(p,'market-product-image')}</div><div class="market-product-copy"><span>${esc(franchiseById(p.franchise).name)}</span><strong>${esc(displayName(p))}</strong><small>${t>=1.25?'🔥 Demand surging':t>=1.1?'↗ Interest rising':'Steady demand'} • ${(t*100).toFixed(0)} index</small></div></button>`).join('')}</div>
      </section>
      <section class="section"><div class="section-title-row"><div class="section-title">Customer chatter</div><div class="section-sub">RECENT SIGNALS</div></div><div class="feed-list">${app.state.marketFeed.slice(0,7).map(renderMarketFeedItem).join('')}</div></section>
  ...
  function renderBusiness(){
    ...
    return `<div class="page-head"><div><div class="eyebrow">BUSINESS</div><h2>Store health</h2><p>Simple numbers first. Deeper systems can come later as your store grows.</p></div></div>
      ...
      <section class="section"><div class="grid-2">
        ...
        <div class="card metric-card"><span>SHELF FILL</span><strong>${Math.round(shelfFill)}%</strong><small>across 9 bays</small></div>
  ...
  function simulateDay(){
    ...
    for(let i=0;i<customerCount;i++){
      ...
      const p=weightedPick(assortment.map(productById), product=>{
        ...
        if(type.id==='parent' && ['Dolls','Plush','Creative','Board Games','Construction','Preschool'].includes(product.category)) w*=1.25;
        if(type.id==='parent' && product.category==='Preschool') w*=1.22;
        if(type.id==='childParent' && ['Action Figures','Die-Cast','Dolls','Blasters','Preschool'].includes(product.category)) w*=1.35;
        if(type.id==='childParent' && product.category==='Preschool') w*=1.18;
      ...
      if(Math.random()>interest) {
        if(Math.random()<.08 && userPrice>p.rrp*1.08) events.push({minute:times[i],type:'chatter',productId:p.id,title:`Customer passed on ${displayName(p)}`,text:`“That feels a little expensive.”`});
        continue;
      }
      if((stock[p.id]||0)<=0){
        lost[p.id]=(lost[p.id]||0)+1;
        events.push({minute:times[i],type:'lost',productId:p.id,title:`Lost sale — ${displayName(p)}`,text:`Customer wanted it, but you were sold out.`});
        continue;
      }
      let qty=1;
      ...
      events.push({minute:times[i],type:'sale',productId:p.id,title:`${displayName(p)} ×${qty} sold`,text:`${money2(userPrice*qty)} sale`});
      if(Math.random()<type.basket){
        ...
        if(attach){
          ...
          if(Math.random()<.55 && stock[attach]>0){stock[attach]--;sales[attach]=(sales[attach]||0)+1;revenue+=apPrice;cogs+=(app.state.inventory[attach]?.avgCost||ap.wholesale);units++;events.push({minute:times[i]+2,type:'sale',productId:ap.id,title:`Added ${displayName(ap)}`,text:`Basket add-on • ${money2(apPrice)}`});}
        }
      }
    }
  ...
  function showTradingModal(result){
    ...
    const tick=now=>{
      ...
      while(idx<result.events.length && result.events[idx].minute<=minute){
        ...
        const ep=e.productId?productById(e.productId):null;
        el.className=`trade-event ${e.type}${ep?' has-product':''}`;
        el.innerHTML=ep
          ? `<div class="trade-event-art-wrap">${artHTML(ep,'trade-event-art')}</div><div class="trade-event-copy"><b>${esc(e.title)}</b><p>${esc(e.text)}</p></div>`
          : `<div class="trade-event-copy"><b>${esc(e.title)}</b><p>${esc(e.text)}</p></div>`;
  ...
  function simulateCompetitors(){
    ...
    DATA.competitors.forEach(c=>{
      ...
      DATA.products.filter(p=>cp[p.id]).forEach(p=>{
        ...
        item.stock-=sold;
        if(item.stock<=1 && Math.random()<.25) moves.push({text:`${c.name} is nearly sold out of ${displayName(p)}.`,productId:p.id});
        if(Math.random()<.055){
          ...
          if(Math.abs(item.price-old)>2) moves.push({text:`${c.name} moved ${displayName(p)} to ${money2(item.price)}.`,productId:p.id});
        }
        if(item.stock===0 && Math.random()<.38){item.stock=Math.max(1,Math.round(p.baseDemand*c.stockBias*rand(3,8)));}
      });
    });
    app.state.rivalMoves=moves.slice(0,6).map(m=>m.text);
    moves.slice(0,3).forEach(m=>app.state.marketFeed.unshift({type:'rival',icon:'🏷️',title:'Rival move',text:m.text,productId:m.productId,time:`After Day ${app.state.day}`}));
  }
  ...
  function generateChatter(){
    ...
    return top.slice(0,2).map(({p})=>({type:'chatter',icon:'💬',title:'Customer chatter',text:pick(templates)(p),productId:p.id,time:`After Day ${app.state.day}`}));
  }
